...
NativeEvents.onEvent("dev.ftb.mods.ftbteambases.events.neoforge.TeamBasesPortalEvent", (event) => {
  try {
    ...
    if (!event.entity.stages.has(portalStage) && !Teams.hasStage(event.entity, portalStage)) {
      let spawn = event.entity.level.getSharedSpawnPos();
      event.entity.level.server.scheduleInTicks(1, () => {
        event.entity.teleportTo(spawn.x, spawn.y, spawn.z);
      });
...
let entityOverworldBlacklist = ["mecrh:ender_chicken"];
...
ServerEvents.commandRegistry((event) => {
  ...
  let root = Commands.literal("ignoreftbpackwarnings")
    ...
    .executes((ctx) => {
      ...
      ctx
        ...
        .tell(
          Text.translate(
            "ftb.teams.bases.portal.ignore_command",
            Text.of(data.contains("ignore_ftb_pack_warnings"))
          ).bold()
        );
      return 1;
    });
